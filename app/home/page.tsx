...
export default function HomePage() {
  ...
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["Endeksler", "Hisseler", "Coinler", "Döviz", "Emtia"]);
  ...
  const handleSearchKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Use the same fallback check logic for the URL to avoid hardcoded ports if running on mobile
      const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3030';
      const res = await fetch(`${API_URL}/api/search?symbol=${encodeURIComponent(q)}`, {
        headers: {
      ...
      if (data.success && data.data) {
        setSearchResult({
          ...data.data,
          formattedPrice: data.data.price?.toLocaleString('en-US', { style: 'currency', currency: 'USD' }) || '—',
          formattedChange: data.data.changePercent !== undefined
            ? (data.data.changePercent >= 0 ? "+" : "") + data.data.changePercent.toFixed(2) + "%"
            : "0%"
        });
      }
    } catch (e) {
  ...
  const handleSearchResultClick = () => {
    if (searchResult) {
      setSelected({
        ...
        price: searchResult.formattedPrice,
        changePercent: searchResult.formattedChange,
        isPositive: searchResult.changePercent >= 0,
        category: "Arama Sonucu",
        description: "Bu varlık Yahoo Finance üzerinden anlık olarak getirilmiştir.",
        group: "Arama Sonucu"
      });
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchAssetsData = async () => {
      ...
      try {
        const token = await getJwtToken();
        const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://192.168.20.152:3030';
        ...
        if (result.success && result.data) {
          baseInstruments = result.data.map((asset: any) => {
            ...
            else if (asset.asset_type === 'index') groupName = "Endeksler";

            const isUsd = asset.asset_type === 'crypto' || (asset.symbol.includes('USD') && asset.symbol !== 'USDTRY') || asset.symbol === '^NDX';
      ...
      <div className="flex flex-1 min-h-0 max-md:flex-col">
          ...
          <div className="bg-white p-6 shadow-sm dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex flex-col flex-1">
            <div className="mb-4">
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                {selected ? `${selected.symbol} için ${activeRange === '1d' ? 'son 1 günlük' : activeRange === '5d' ? 'son 5 günlük' : activeRange === 'ytd' ? 'yılbaşından bu yana olan' : `son ${RANGES.find(r => r.value === activeRange)?.label.toLowerCase()}lık`} görünüm` : "Genel piyasa görünümü (Örnek Veri)"}
              </p>
            </div>
        ...
        >
          <div className="flex flex-col border-b border-zinc-200 dark:border-zinc-800 flex-1 min-h-0 max-md:flex-none max-md:h-[400px] max-md:shrink-0">
            <div className="px-4 py-2.5 border-b border-zinc-100 dark:border-zinc-800 flex flex-col gap-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-4 py-1.5 text-[11px] bg-zinc-100 dark:bg-zinc-800/80 border-none rounded-lg focus:ring-1 focus:ring-zinc-300 dark:focus:ring-zinc-700 outline-none text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-500 dark:placeholder:text-zinc-400 transition-all font-medium"
                />
              </div>

              {/* Search result card */}
              {isSearching ? (
                <div className="w-full flex items-center justify-center py-4 mt-1">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-2.5 mt-1 rounded-lg bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-700/70 transition-all shadow-sm text-left"
                >
                  <div className="flex flex-col min-w-0">
                    <span className="text-xs font-bold text-zinc-900 dark:text-white truncate">
                      {searchResult.symbol}
                    </span>
                    <span className="text-[10px] text-zinc-400 truncate">{searchResult.name}</span>
                  </div>
                  <div className="flex flex-col items-end ml-3 shrink-0">
                    <span className="text-xs font-semibold text-zinc-800 dark:text-zinc-100">{searchResult.formattedPrice}</span>
                    <span className={`text-[10px] font-bold ${searchResult.changePercent >= 0 ? "text-emerald-500" : "text-red-500"}`}>{searchResult.formattedChange}</span>
                  </div>
                </button>
              ) : null}
            </div>
